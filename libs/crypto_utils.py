...
def base64_to_pem(pem_string):
    ...
    return serialization.load_pem_public_key(
        base64.b64decode(pem_string.encode("utf-8")), backend=default_backend()
    )
...
def decrypt_message(decrypted_symm_key, message, iv):
    ...
    )  # Convert to bytes first, then decode from base64
...
def encrypt_symm_keys(symm_key, *recipients):
    ...
    for recipient in recipients:
        ...
        public_key = serialization.load_pem_public_key(
            decoded_key, backend=default_backend()
        )
...
def sign_data(private_key, data):
    """Signs the provided data with the client's private key.

    Args:
        private_key: The RSA private key object.
        data: The data to be signed (in bytes).
